...
public class SQLselect1 extends MyConnection{
	...
	executeQuery()
		: select 쿼리문을 실행한다. 반환값은 조회할 레코드이므로
		  ResultSet 인스턴스가 반환된다. Java에서 반환 받은 후 while문으로 개수만큼
		  반복하여 인출한다.
	 */
	@Override
	public void dbExecute() {
		try{
			...
			while(rs.next()) {
				...
				출력된다. 0000-00-00 00:00:00 형식이므로 적당한 크기로 잘라야한다면
				substring()을 사용하면 된다.
				 */
				...
				String regidate3 = rs.getString("regidate")
										.substring(0,13);
				/*
				오라클의 변환함수를 통해 포매팅한 후에 문자형식으로 출력한다.
				쿼리문에 별칭을 사용한 경우 getString()에서 그대로 사용할 수 있다.
				 */
