...
public class SQLinsert extends MyConnection {
	...
	/*
	이 멤버변수는 생성자에게 초기화되지 않는다. 생성자는 대부분
	멤버변수의 초기화를 목적으로 정의되지만, JDBC프로그래밍에서는 
	쿼리문을 실행해야하므로 인스턴스가 생성된 후 변수를 초기화해야한다.
	 */
	...
	public void dbExecute() {
		try {
			/*
			Statement인터페이스
			: 정적 쿼리문을 실행할 때 사용한다. 정적 쿼리문이란 인파라미터가 없는
			  완성된 쿼리문을 말한다. 수정은 개발자가 직접 진행해야한다.
			 */
			stmt = con.createStatement();
			// 쿼리문 작성
			query = "INSERT INTO member VALUES('test3','1111','테스터3',sysdate)";
			...
			: 인파라미터가 있는 동적 쿼리문을 실행할 때 사용한다.
			  인파라미터는 ?로 표시하고, 인스턴스 생성 이후 setXX() 메서드를 통해
			  설정한다. 인덱스는 1부터 시작한다.
			 */
