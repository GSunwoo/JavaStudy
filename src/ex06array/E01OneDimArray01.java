...
-자바에서는 배열을 인스턴트 형태로 생성하므로 new를 사용한다.
-생성된 배열은 힙(Heap)의 영역에 저장되고, 할당된 메모리의 주소값을 반환한다. 
