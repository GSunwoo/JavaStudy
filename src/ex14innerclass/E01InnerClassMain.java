...
/*
내부클래스(Inner class)
: 클래스 안에 정의된 클래스로 static이 없는 클래스를 말한ㄷ.
...
class Unit {
	...
	// 인스턴스형 멤버 변수 : 사용시 new를 통해 인스턴스 생성
	Marine marine;
	Medic medic;
...
public class E01InnerClassMain {
	public static void main(String[] args) {
		...
		직접 인스턴스를 생성할 수 없다.
		 */
		//Marine m = new Marine(); // error

		/*
		외부클래스의 인스턴스를 먼저 생성한 후 내부클래스의 인스턴스를 생성해야한다.
		Marine은 Unit클래스의 생성자에서 이미 인스턴스 생성이 되어있으므로
		이와같이 바로 사용하면 된다.
		 */
		...
		/*
		Medic은 내부에서 생성되지 않았으므로 new를 두 번 사용해서 인스턴스를 생성하면
		된다.
		 */
