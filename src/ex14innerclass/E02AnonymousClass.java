...
-마지막에 세미콜론을 기술해야한다.
형식]
	부모클래스 참조변수 = new 부모 클래스명() {
		익명클래스의 실행부;
		부모클래스의 메서드 오버라이딩;
	};
	※부모클래스를 상속받아 이름없는 자식클래스를 생성하는 개념으로 이해하면 된다.
	 상속되기 때문에 오버라이딩이 가능하다. 즉, 중괄호 안쪽은 "자식"의 영역이다.
 */
...
class Singer{
	...
	Java에서 정의하는 모든 클래스는 Object클래스를 상속하므로 이와같이 오버라이딩 할 수 있다.
	 */
	@Override
...
public class E02AnonymousClass {
	...
	public static void main(String[] args) {
		...
		/*
		인스턴스의 참조값을 할당받은 참조변수를 출력한다. 이때 오버라이딩한 toString의 반환값이
		출력된다. 만약 toString을 삭제하면 @를 포함한 참조값이 출력된다.
		 */
		...
		/*
		부모의 참조변수로 자식을 참조할 수 있지만, 접근범위가 부모까지로 제한된다.
		따라서 아래와 같이 자식에서 확장한 멤버변수는 접근할 수 없어 에러가 발생한다.
		접근하고 싶다면 다운캐스팅(강제형변환)을 해야하지만 익명클래스는 이름이 없기때문에
		다운캐스팅 자체가 불가능하다. 따라서 익명 클래스에서는 멤버의 학장은 거의 사용하지 않고
		대부분 오버라이딩만을 목적으로 활용된다.
		 */
//		System.out.println(s3.mic); //error
		System.out.println(s3);
		
		/*
		인터페이스를 구현해서 익명클래스를 정의한다. 오버라이딩이 주된 목적이므로, 부모클래스에는
		추상메서드만 정의되어 있어도 아무 상관이 없다.
		 */
