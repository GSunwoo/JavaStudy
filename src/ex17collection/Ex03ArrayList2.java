...
public class Ex03ArrayList2 {
	...
	public static void main(String[] args) {
		...
		Student 형의 타입매개변수를 통해 생성한다. Student를 저장할 수 있는 상자가 된다.
		 */
		LinkedList<Student> list2 = new LinkedList<Student>();
//		ArrayList<Student> list2 = new ArrayList<Student>()
		
		/*
		저장할 Student 인스턴스 생성. 외부패키지에 선언되었으므로 import 후에 사용할 수 있다.
		 */
		...
		또한 동일한 인스턴스가 존재한다면 앞에서부터 순서대로 삭제된다.
		즉, 동시에 삭제되지 않는다.
		 */
		...
		for (Student st: list2) {
			/*
			이름으로 검색시 멤버변수 name은 private으로 선언되었으므로 getter를 통해 접근해야한다.
			삭제를 위해 입력한 이름과 Student인스턴스에 저장된 name과 일치하는지 확인한 후 
			인스턴스를 검색할 수 있다.
			 */
		...
		인스턴스의 인덱스를 통해 삭제하면 참조값이 반환된다. 따라서 삭제된 인스턴스의 정보를 
		출력할 수 있다.
		 */
