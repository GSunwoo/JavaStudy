...
-인스턴스가 순서없이 저장된다.
-중복저장을 허용하지 않는다.
-기본클래스(String, Integer등)인 경우에는 별도의 처리없이 중복이 제거된다.
-하지만 개발자가 정의한 클래스라면 hashCode(), equals()를 오버라이딩해야한다.
...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		/*
		3가지의 기본클래스와 우리가 정의한 Teacher의 인스턴스를 생성한다.
		 */
		String varString1 = "JAVA";
		String varString2 = new String("개발자");
		Date varDate = new Date();
		int varInt = 100; // 저장시 Interger로 Boxing 처리됨
		Teacher varTeacher = new Teacher("김봉두", 55, "체육");
		
		/*
		객체저장
		: add()메서드를 통해 저장한다. 저장에 성공한 경우 true를 반환한다.
		 */
		...
		// size()를 통해 저장된 데이터의 개수를 알 수 있다.
		System.out.println("[중복 저장전 객체수]:" + set.size()); // 5
		
		/*
		기본클래스의 객체 중복저장
		: 기본 클래스의 인스턴스인 경우 별도의 오버라이딩 없이도 중복이 제거된다.
		  따라서 아래 add()는 false를 반환한다.
		 */
		...
		while(itr.hasNext()) {
			...
			/*
			HashSet에 저장시 모두 Object형으로 자동형변환되어 저장되므로 우리가 직접
			정의한 클래스에 대해서만 instanceof를 통해 타입을 확인한 후 다운캐스팅하여 출력한다.
		...
		/*
		포함여부 확인
		:set에 해당 인스턴스가 저장되어 있는지 확인하여 boolean을 반환한다.
		 */
		System.out.println(set.contains(varString1)?"varString1 있음":"varString1 없음");
		
		/*
		객체 삭제
		: set은 인덱스를 사용하지 않으므로 인스턴스의 참조값을 통해 삭제한다.
		 */
