...
-set의 기본적인 특성은 동일하다.
-따라서 중복은 허용하지 않는다.
-단, 데이터가 정렬되어 저장된다. 정렬의 기준은 개발자가 직접 정의할 수 있다.
 */

/*
클래스 정의시 정렬의 기준을 만들어주기 위해 Comparable<T> 인터페이스를 구현해야한다.
 */
class MyString implements Comparable<MyString> {
	...
	/*
	TreeSet<T>에 인스턴스를 저장할때 정렬을 위해 compareTo를 오버라이딩한다.
	정렬의 기준은 단순히 알파벳 순서가 될 수도 있고, 문자열의 길이에 따른 정렬이 될 수도 있다.
...
//			return 0;
//		}
		if(this.str.charAt(0) > o.str.charAt(0)) {
			return 1;
		}
		else {
			return -1;
