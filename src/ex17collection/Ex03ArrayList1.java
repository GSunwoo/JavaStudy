...
- 데이터의 중복 저장이 허용된다.
- 데이터의 저장 순서를 보장한다.
- 데이터 접근시 get() 혹은 iterator를 사용한다.
- Array라는 이름처럼 '배열'의 특성을 가지고 있어 index를 통해 저장 및 접근이 가능하다.
 */
...
//		Vector<String> list = new Vector<String>();
		...
		/*
		인스턴스저장 : add()
			add(인스턴스) : 순차적으로 저장하면서 인덱스는 0부터 자동으로 부여된다.
			add(인덱스, 인스턴스) : 데이터 저장시 인덱스를 직접부여한다.
							   단, 인덱스를 건너뛰면 에러가 발생되므로 주의해야한다.
							   저장하려는 인덱스에 이미 저장된 데이터가 있다면 삽입(끼워넣기)이
							   된다.
		 */
		list.add("소녀시대");
		list.add("빅뱅");
		list.add("트와이스");
		list.add(3,"워너원");
//		list.add(5,"소방차"); // error
		/*
		size() : List에 저장된 인스턴스의 수(크기) 반환. 
		...
		/*
		중복저장
		: List는 배열의 특성을 가지므로 데이터를 중복저장할 수 있다. 동일한 데이터라도 index로
		...
		출력하기
		방법1
		: 일반 for문 사용. 이때는 인덱스를 통해 접근해야하므로 get() 메서드를 사용한다.
		 */
		System.out.println("\n[일반for문 사용]");
		...
		/*
		방법2
		: 확장 for문의 경우 인덱스없이 전체 원소를 반복해주므로 좀 더 간단한 표현이 가능하다.
		  따라서 가장 많이 사용된다.
		 */
		System.out.println("\n[확장for문 사용]");
		for(String obj : list) {
		...
		/*
		Iterator 사용법
		1. 컬렉션에 저장된 내용을 Iterator에게 알려주기 위해 인스턴스를 생성한다.
		...
		/*
		수정하기(덮어쓰기)
		: 기존의 내용을 변경한다. 즉, 기존의 내용은 삭제 후 새로운 내용으로 갱신한다.
		 */
		...
		/* 
		방법1 - 인덱스로 삭제
		: 인덱스를 통해 삭제한 후 완료되면 해당 인스턴스의 참조값을 반환한다. 삭제후에는 전체적인
		  인덱싱이 자동으로 진행된다.
		 */
		...
		/*
		방법2 - 인덱스로 삭제
		: 방법1과 동일하게 인덱스로 삭제한다. 단, indexOf로 인스턴스의 인덱스를 찾은 후 이값을 
		  사용한다.
		 */
		...
		: 이 경우에는 삭제에 성공할 경우 true가 반환된다. 즉, boolean타입의 반환값을 가진다.
		 */
		// 삭제 성공
		...
		/*
		전체 삭제
		: 아래 2개의 메서드를 통해 컬렉션에 저장된 전체 데이터를 삭제할 수 있다.
		 */
		list.removeAll(list);
		list.clear();
