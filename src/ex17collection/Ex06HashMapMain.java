...
public class Ex06HashMapMain {
	...
	public static void main(String[] args) {
		...
		/*
		객체저장
		: 저장시 기존에 저장된 동일한 Key값이 존재하면 저장된 인스턴스가 반환된다. 만약 처음이라면
		  null을 반환한다.
		 */
...
//		map.put("age", number);
		...
		/*
		중복저장
		: 기존에 입력된 name이라는 Key가 있기 때문에 "홍길동"이 반환된다. 그리고 기존의 값은
		  "최길동"으로 수정된다.
		 */
		System.out.println("name이라는 키값으로 저장된 이전의 값: "+ 
				map.put("name", "최길동"));
		...
		//출력하기
		/*
		1.키값을 알고 있을때 : get(key값)으로 출력
		 */
		System.out.println("키값을 알때:" +map.get("name"));
		
		/*
		2. 키값을 모를때(혹은 너무 많아서 명시하기 힘들때) 
		: keySet()을 통해 전체 key를 Set으로 얻어온다. 이 값을 통해 반복문으로 접근하여
		  value를 인출할 수 있다.
		 */
		...
		/*
		3. 이터레이터를 통한 출력
		: 2와 동일하게 keySet()으로 얻어온 후 반복 출력한다.
		 */
		...
		//Key가 있는지 확인 후
		while(it.hasNext()) {
			// 존재하면 Key를 얻어와서 value를 인출
			String key = it.next();
			String value = map.get(key);
			System.out.println(String.format("%s:%s" ,key ,value));
		}
		/*
		values() 메서드를 통해 값만 얻어올 수 있다. 컬렉션의 형태로 얻오온 후 반복해서 출력한다.
		 */
		...
		/*
		존재유무확인
		: Map은 key와 value가 있으므로 메서드도 2개로 나누어져 있다.
		 */
		...
		/*
		전체삭제
		: removeAll()은 없고, clear만 사용할 수 있다.
		 */
		map.clear();
