...
public class Teacher extends Person{
	...
	: 인스턴스가 가진 고유한 주소값을 정수형으로 반환해준다.
	  ...
	  정수형 멤버변수는 별도의 처리없이 사용하면되고, 인스턴스형 멤버변수는 
	  hashCode()메서드를 통해 정수값으로 반환되는 참조값을 사용하면 된다.
	 */
	@Override
	public int hashCode() {
		
		//방법1 : 정수형변수와 인스턴스의 해시코드값을 합산한 후 그대로 반환한다.
		int returnCode1 = super.getAge() + this.subject.hashCode();
		System.out.println("returnCode1="+returnCode1);
		/*
		방법2 
		: Objects 클래스의 정적인메서드인 hash()를 사용한다.
		  멤버변수의 갯수만큼 인수로 전달하면 방법1과 동일하게 고유한 주소값을 정수형으로 반환해준다.
		 */
	...
	/*
	equals()
	: 인스턴스가가진 멤버변수의 값을 비교한다. 값이 같으면 true, 다르면 false를 반환하도록 오버라이딩
	 */
	@Override
	public boolean equals(Object obj) {
		...
		비교해야한다. 즉, 우리가 정한 기준에 따라 멤버변수끼리 값이 동일한지 비교하도록 오버라이딩하면된다.
		 */
