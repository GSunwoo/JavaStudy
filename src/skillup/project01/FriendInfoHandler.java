...
/*
주소록 관리 프로그램에서 기능을 담당하는 핸들러 클래스 정의.
main메서드는 프로그램의 출발점의 역할만 담당하고, 모든 기능을 핸들러
클래스에서 구현한다. */
import java.util.Scanner;

public class FriendInfoHandler {
	/*
	연락처를 저장하기 위한 인스턴스형 배열을 생성. 고딩 혹은 대딩 각각의
	배열이 필요하고, 카운터를 위한 변수도 생성한다. */
	...
	public void addFriend(int choice) {
		...
		if(choice==1) {
			...
			/*
			인스텀스 참조값을 배열에 추가. 카운트용 변수를 후위 증가 시켜
			0번 인덱스에 먼저 입력된 후 1 증가한다. */
	...
	public void showAllData() {
		//System.out.ptintln("## showAllData 호출됨 ##");
		/*고딩 친구 반복 : 저장된 정보의 객수만큼 반복해서 전체정보를 출력한다.
		4개의 멤버변수 모두 출력한다. */
	...
	public void searchInfo() {
		...
		for(int i=0 ; i<numOfhighFriend ; i++) {
			/*
			문자열 비교를 위한메서드 중 compareTo()를 사용해서
	...
	public void deleteInfo() {
		//Systrm.out.println("## deleteInfo 호출됨 ##");
		Scanner scan = new Scanner(System.in);
		System.out.print("삭제할 이름을 입력하세요:");
		String deleteName = scan.nextLine();
		/*
		삭제될 인스턴스의 인덱스를 저장하기 위한 변수. 배열의 인덱스ㅡㄴ 
		...
		//고딩친구 반복
		for(int i=0 ; i<numOfhighFriend ; i++) {
			/*삭제할 이름과 일치하는 인스턴스를 찾기 위해 equals()를 사용.
			일치할 떄 true를 반환한다. */
			if(deleteName.equals(highFriends[i].name)) {
				/*
				일치하는 이름을 찾았따면 해당 인덱스에 null를 입력
				한다. 참조값이 null이라는 것은 참조할 인스턴스가 없다는 의미이므로
				삭제라고 볼 수 있다. */
				highFriends[i] = null;
				//삭제된 인스턴스의 인덱스를 저장
				deleteIndexHigh = i;
		...
		if(deleteIndexHigh!=-1) {
			...
			//고딩, 대딩 모두 -1을 유지한다면 검색된 값이 없다는 의미. 나메나메나메나메
			System.out.println("## 없는디?;; ##");
		}
	}
}
