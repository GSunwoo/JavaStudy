...
/*
equals() 메서드
- 자바에서 인스턴스의 내용 비교를 위해 비교연산자(==)를 사용하는 것은 단순히 참조값에 대한
  비교를 하는 것이므로, 실제 내용에 대한 비교가 이루어지지 않는다.
- 인스턴스 간의 내용 비교를 위해서는 equals() 메서드를 각 클래스에 맞게 오버라이딩하여 사용해야한다.
...
class IntNumber {
	...
	/*
	equals() 메소드가 어떻게 인스턴스간의 내용을 비교하는지 설명하기 위한 코드를 기술함
	 */
...
public class E04Object2MyEquals {
	...
	public static void main(String[] args) {
		...
		/*
		개발자가 직접 정의한 myEquals를 통해 인스턴스간의 내용비교를 한다.
		여기서는 다르다고 나온다.
		 */
		...
		/*
		인스턴스간의 비교를 할때는 단순히 참조변수에 대한 비교를 하면 안되고
		실제 내용에 대한 비교를 해야한다. 즉 멤버변수끼리 비교해서 판단해야한다.
		 */
