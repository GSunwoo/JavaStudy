...
//		System.out.println("120원: " + Integer.parseInt(money)); // 예외발생
		...
		/*
		실수형태의 문자열을 숫자로 변경할때는 parseInt()는 사용할 수 없다. 
		앞과 동일한 예외가 발생된다.
		 */
...
//		System.out.println(Integer.parseInt(floatString));
		...
		/*
		codePointAt()
		: 문자열에서 특정 index에 해당하는 한 문자의 아스키코드를 반환한다.
		 */
		...
		/*
		isDigit()
		: 인자로 전달된 문자가 숫자인지 판단한다. 숫자일때 true를 반환한다.
		 */
		...
		/*
		isLetter()
		: 문자인지 여부를 판단하는 메서드로 특수기호나 숫자형일때는 false를 반환한다.
		 */
		...
		/*
		isWhitespace()
		: 공백문자인지 판단하는 메서드. 공백문자일때 true를 반환한다.
		 */
		...
		*/
		...
		char[] strChar = str.toCharArray(); // 입력받은 문자열을 char형 배열로 변환
		
		int ctr = 0; // 공백의 수를 카운트할 변수
		...
		System.out.printf("공백문자 수 : %d", ctr);
	}
}
