...
public class E05Date {
	/*
	Calendar 클래스를 통해 얻은 인스턴스르 초(Sec)로 변환한 후 매개변수로 전달된 날짜서식으로
	변환한 문자열을 반환한다.
	 */
	public static String myDateFormat(Calendar c, String formatStr) {
		/*
		서버의 표준시인 1970-01-01부터 현재까지의 시간을 초 단위로 반환해주는 메서드로 이를
		'타임스템프'라고 한다.
		 */
		Date date = c.getTime();
	...
	public static void main(String[] args) {
		/*
		Calendar 클래스는 Java의 대표적인 싱글톤 객체이므로 new를 사용하는 대신 유틸리티 메서드인
		getInstance()를 통해 참조값을 얻어온다.
		 */
		Calendar calendar = Calendar.getInstance(); 
		
		/*
		월을 표현하는 Month, 분을 표현하는 Minute의 첫글자가 동일하므로 구분을 위해 월은 대문자,
		분은 소문자를 사용한다.
		 */
		...
		/*
		현재 날짜를 기준으로 과거/미래를 구하기 위해 add메서드를 사용한다.
		두번째 인자가 음수면 과거의 날짜를 구할 수 있다.
		 */
		calendar.add(Calendar.DATE, 3);
		calendar.add(Calendar.MONTH, 2);
		calendar.add(Calendar.YEAR, 1);
		/*
		앞에서 add를 3번 호출했으므로 결과는 아래와 같이 1년 2개월 3일후의 날짜가 출력된다.
		 */
		String nowStr3 = myDateFormat(calendar, "yyyy년 MM월 dd일");
		System.out.println("1년2개월3일후: " + nowStr3);
		
		/*
		올해 크리스마스까지 남은 날짜를 계산. 여기서는 두가지 서식의 날짜를 파싱하여 구한
		날짜 인스턴스로 계산한다.
		 */
		// 오늘날짜 : 0000-00-00 형식
		String today = 
