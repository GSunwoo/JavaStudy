...
public class Ex02PreDefineException1 {
	...
	public static void main(String[] args) {
		...
		/*
		예외가 발생할 수 있는 지역을 try로 먼저 묶어주고 예외가 발생되면 JVM(자바가상머신)은
		해당 예외에 관련된 이스턴스를 만들어 프로그램쪽으로 전달한다.
		이때 catch절에서 예외 인스턴스를 잡아 주는 것을 "예외처리"라고 한다.
		만약 이 예외가 catch되지 못하면 프로그램은 비정상적인 종료로 이어지게 된다.
		 */
		try {
			intArr[0] = 100;
			...
			/*
			예외가 발생하는 지점.
			해당 코드가 실행될 때 예외가 발생하게 되고 JVM이 예외 인스턴스를 생성해서 프로그램으로
			전달한다.
			 */
			intArr[2] = 300;
			...
			// 예외메세지3 : 예외에 대한 디테일한 설명을 출력(가장 많이 사용됨)
			e.printStackTrace();
		}
