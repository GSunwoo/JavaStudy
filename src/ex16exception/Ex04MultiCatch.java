...
/*
예외처리를 할때 필요에 따라 여러개의 catch블럭을 사용할 수 있다. 이 경우 반드시 자식 예외 클래스부터
catch 블럭에 추가해야한다. Exception 클래스는 모든 예외를 처리할 수 있으므로 반드시 마지막 블럭에
사용해야한다.
	...
	public static void main(String[] args) {
		...
		try {
			...
			for(int i =0; i<array.length;i++) {
				...
				단, "1a"와 같이 입력하면 정수로 변경할 수 없으므로 예외가 발생된다.
				 */
			...
			/*
			배열의 크기가 3이므로 인덱스는 2까지만 존재한다. 따라서 이 코드도 예외가 발생된다.
			 */
			array[3] = age;
		}
			...
			System.out.println("숫자형태의 문자만 가능합니다.");
		}
		...
		/*
		앞 부분에서 예외가 발생하더라도 예외처리를 했으므로 프로그램은 끝까지 실행된 후
		정상적으로 종료된다. 실행의 흐름이 마지막까지 이어지게 하는 것이 "예외처리"의 목적이다.
		 */
