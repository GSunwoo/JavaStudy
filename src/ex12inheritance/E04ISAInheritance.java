...
따라서 실제 업무에서도 유사한 기능의 클래스들을 상속으로 묶어서 정의하는 것이 좋다. */

//컴퓨터 : 기본적인 컴퓨팅화면을 제공하여 입력된 내용을 계산한다.
class Computer{
	//멤버변수 : 소유주
	String owner;
	...
	/*
	매개변수가 있는 인자생성자 추가.
	이 경우 디폴트 생성자는 추가되지 않는다.
	 */
	public Computer(String name) {
		owner = name;
	}
	/*
	private 접근지정자로 선언된 멤버메서드는 외부클래스 혹은 상속받은 클래스에서 접근이 불가능하므로, 
...
class NotebookComputer extends Computer {
	...
	public NotebookComputer(String name, int initCharge) {
		/*
		부모클래스의 생성자를 호출하는 super()는 생략할 수 있비만,
		앞에 다른 문장임먼저 기술되면 에러가 발생한다. 
		항상 부모의 생성자를 최우선으로 호출해야 한다. */
		super(name);
		battery = initCharge;
	}
	//충전을 표현
	...
	public void movingCal() {
		//사용 전 배터리를 확인한 후 사용여부를 판단
		if(battery < 1) {
			/*
			방전된 상태라면 사용이 불가하므로 return을 통해
			해당 메서드의 실행을 중지한다.
			 */
		...
		/*
		아래 2개의 메서드는 private로 선언되었으므로 
		상속받은 자식 클래스에서도 접근할 수 없어 에러가 발생한다.
		이 경우 public으로 선언된 메서드를 통해 간접 호출해야한다.
		 */
		//KeyboatdTouch();    //에러
		//calculate();        //발생
		calculateFunc();
...
/*'
컴퓨터를 상속한 노트북
: 컴퓨터가 가진 기본 계산기능에 휴대성을 확장하여 정의한다.
휴대하기 위해서 배터리가 필요하므로 멤버변수로 확장하였다. */

/*
테블릿 : 컴퓨터의 기본 계싼 기능과, 후대이용 그리고 전용펜으로
...
class TabletNotebook extends NotebookComputer{
	...
	public void write(String penInfo) {
		//배터리가 충준한지 확인
		if(battery<1) {
			System.out.println("배처리가 방전되어 사용 불가");
			return;
		}
		/*
		...
		/*
		부모클래스에 정의된 메서드를 호출한다.
		상속받은 자식 클래스에서는 이와같이 부모의 멤버메서드를 호출하여
		자신의 기능처럼 사용할 수 있다. 단 접근이 가능한 지정자로 선언되어야 한다.
		super는 생략할 수 있다. */
		super.movingCal();
...
public class E04ISAInheritance {
	...
	public static void main(String[] args) {
		...
		noteBook.movingCal();
		noteBook.movingCal();
		noteBook.movingCal();//배처리가 1씩 차감되므로 여기가지 사용 가능
		noteBook.movingCal();//배터리가 모두 소모되어 사용 불가
		noteBook.movingCal();
		noteBook.movingCal();
