...
오버로딩 : 메서드명은 같지만 매개변수의 타입, 갯수, 순서가, 다른 경우를
	말한다. 즉, 서로 다른 메서드이므로 상속받은 하위클래스에는 오버로딩된
	모든 메서드가 포함된다.
오버라이딩 : 메서드명, 매개변수, 반환형까지 모두 동일한 메서드를 상속관계에 있는
	하위클래스에서 재정의한 것을 말한다.
	이 경우 하위클래스에서 정의한 메서드가 상위클래스의 메서드를가리게 되므로,
	항상 최하위 클래스의 메서드가 호출된다. 즉 오버라이딩된
...
class C extends B {
	/*
	ride메서드는 오버라이딩 정의ㄷ된 메서드로 상위클래스에서 정의된
	...
	/*
	load메서드는 오버로딩으로 정의했으므로 메서드명만 동일할 뿐 매개변수의
	타입, 갯수 등이 다르다. 따라서 서로 다른 메서드이다. 교실개추워ㅜㅜ*/
	public void loadMethod(double num) {
		System.out.println("C의 loadMethod");
	}
	/*
	따라서, C클래스러 인스턴스를 생성하면 오버라이딩된 메서드는 모두
	가려지므로 1개만 남게되고, 오버로딩된 메서드는 3개 모두 존재하게 된다. 
	따라서 C인스턴스에는 4개의 메서드가 존재한다. 졸린나메코Zz*/
}
public class E02RideAndLoad {
	...
	public static void main(String[] args) {
		...
		/*
		부모타입의 참조변수로 자식인스턴스를 참조할 수 있으므로
		아래 문장은 모두 성립된다. */
		A ref1 = new C();
		B ref2 = new C();
		C ref3 = new C();
		...
		/*
		C타임의 참조변수를 통해서인스턴스를 참조하면 모든 영역에
