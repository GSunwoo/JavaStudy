...
완전히 동일한 모양으로 메서드를 재정의 하는것를 말한다.
...
class BaseSpeaker extends Speaker {
	...
	/*
	@Override : 이노테이션이라 부르고, 오버라이딩된 메서드세 추가하여
	컴파일러에게 알려주는 역할을 한다. 필수는 아니므로 생략할 수 있다. */
	@Override
	public void showState() {
		/*
		super를 통해 부모클래스에 정의된 멥버메서드를 호출한다.
...
public class E01Overriding {
	...
	public static void main(String[] args) {
		...
		호출된다. 부모쪽의 메서드는 자식에 의해 가려진다. */
		baseSpeaker.showState();
		
		System.out.println();
		
		/*
		부모클래스의 참조변수로 자식 인스턴스를 참조할 수 있다.
		이것은 상속관게이기 때문에 가능띠하다. */
		Speaker speaker = new BaseSpeaker();
		/*
		단, 부모타입의 변수로 자식 인스턴스를 참조하면 접근할 수 있는
		범위가 부모인스턴스로 제한된다. 따라서 자식쪽에 정의된 setBase()
		는 호출할 수 있다. */
