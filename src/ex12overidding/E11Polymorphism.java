...
/*
다형성(polymorphism)
: 하나의 인스턴스(객체)를 여러 타입의 참조변수를 통해 참조할 대
참조변수의 타입에 다라 다양한 형태의 인스턴스를 이용할 수 있는 특성을 말한다.

~부모 타입의 참조변수로 자식인스턴스를 참조
1. 이 경우 부모타입의 참조변수는 부모로부터 상속받은 멤버가지만 접근할 수 있다.
...
2, 이것을 동질화(Homogeneous:호모지니어스)라고 표현한다. 붕방방방부왕~
 */
...
class MyParent {
	int parentMember;
	void parentMethod() {
		System.out.println("부모의 메소드:patentMethod()");
	}
}
...
class MyChild extends MyParent{
	...
	@Override
	void parentMethod() {
		System.out.println("자식에서 overrinding한 메소드"
				+ "parentMethod()");
	}
	/*
	메서드명은 동일하지만 매개변수가 틀리므로 오버로딩으로 정의한 메서드
	따라서 자식에서 확장한 멤버가 된다. */
	void parentMethod(int childMamber) {
		this.childMember = childMamber;
...
public class E11Polymorphism {
	...
	public static void main(String[] args) {
		...
		접근하고 싶다면 자식타입으로 강제형변환(다운캐스팅)해야
		한다. */
		...
		//부모 참조변수로 자식 인스턴스를 참조한 두 번째 인스턴스 생성
		...
		소괄호를 하나 더 추가해서 myPerant를 자식타입으로 형변환 후
		멤버변수에 접근해야한다.
		 */
		((MyChild)myParent).childMember = 1;
		...
		/*
		Java애서 생성한 모든 클래스는 Object클래스를 상속한다.
		다라서 모든 인스턴스는 Object의 참조변수로 참조가 가능하다.
		도한 Object 클래스에 정의된 모든 메서드를 별도의 선언없이 사용할 수 있고, 
		필요하다면 오버라이딩도 할 수 있다. */
