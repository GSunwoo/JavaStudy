...
public class E05InstanceOf {
	...
	/*
	-메서드 정의 시 부모타입의 매개변수로 설정하면 자식 타입의 인수를
	한거번에 받을 수 있다.
	-Box타입으로 인수를 받으므로 자동형변환(업캐스팅)되어 자식 클래스의
	멤버메서드를 호출할 수 없게된다.
	-이 때 자식의 멤버메서드를 호출하기 위해 강제형변환(다운캐스팅)이
	필요하다.
	-instanseof 연산자를 통해 자식타입으로 형변환이 가능한지 판단한 후
...
//	static void wrapBox(GoldPaperBox b) {}
	...
	public static void main(String[] args) {
		...
		/*
		String 인스턴스는 Box와는 아무런 관계가 없으므로 인수로
		전달할 수 없다. 즉 상속관계가 성립하 않는다. */
