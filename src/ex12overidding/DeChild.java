...
public class DeChild extends DeParent {
	...
	/*
	오버라이딩 아님 : 부모클래스에서 private멤버로 선언하였으므로
	자식쪽에서는 보이지 않아 오버라이딩의 대상이 될수없다.
	이 메서드는 자식쪽에서 확장한 메서드가 된다.
	 */
	private void eat() {
	...
	/*
	메서드 오버라이딩의 조건
	-부모클래스에 선언될 대와 비교해서 접근범위가 동일하거나
	그보다 넓으면 가능하다.
	...
	/*
	메서드의 형태가 부모와는 다르므로 오버라이딩이 될수없다.
	...
	2.super.멤버 : 부모의 멤버변수 혹은 멤버변수를 호출한다. */
	@Override
	...
	클래스 외부(메서드영역)에 미리 로드되므로, 오버라디잉이 되지않고
	호출할 때 각 클래스명을 사용하면 된다. */
