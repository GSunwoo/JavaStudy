...
- 추상클래스와 동일하게 상속을 목적을 제작되므로 하위클래스에서는 반드시 추상메서드를
  오버라이딩 해야한다.
...
//	public MyInterface1() {}
	...
	/*
	멤버상수 : 인터페이스에 선언되는 변수는 상수로 선언된다. 따라서 대문자로 작성해야한다.
	 */
...
/*
인터페이스는 콤마를 이요애서 여러개를 '구현'할 수 있다.
또한  상속과 구현을 동시에 사용할 수도 있다.
 */
class MyClass extends SimpleClass implements MyInterface1, MyInterface2 {

	/*
	인터페이스에 동일한 이름의 추상메서드가 존재하는 경우 하나만 오버라이딩 하면 된다.
	...
	/*
	SimpleClass 클래스의 멤버메서드는 오버라이딩이 필수가 아니므로 필요한 경우에만
	추가하면 된다.
	 */
}
